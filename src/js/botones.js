const btnRegresar = document.querySelectorAll('.btnRegresar');
const btnIrHabilidades = document.querySelector('.opcHabilidades');
const btnIrSobreMi = document.querySelector('.opcSobreMi');
const btnIrContacto = document.querySelector('.opcContacto');
const btnPortafolio = document.querySelector('.opcPortafolio');


const framePortafolio = document.querySelector('.framePortafolio');
const frameContacto = document.querySelector('.frameContacto');

const contacto = document.getElementById('contacto');
const sobreMiElement = document.getElementById('sobreMi');
const btnSobreMi = document.getElementById('btnSobreMi');
